...
- [ ] Write failing tests for 32 balanced tasks, episode/cross-harness coverage, unique IDs, pinned commits, evidence offsets, time/tool limits, and 240 non-overlapping gold cases.
- [ ] Encode the six preregistered contrasts, formulas, gates, 10,000 bootstrap resamples, seed, and Holm adjustment in the manifest.
- [ ] Add `current_state` and `candidate` benchmark labels so an enhanced run cannot overwrite or masquerade as the original architecture’s result.
- [ ] Add a machine-readable result state for each goal: `proven`, `inconclusive`, `regressed`, `invalid`, or `not_run`.
- [ ] Run `cargo test -p brain-cli --test second_brain_benchmark_suite`.
...
- [ ] Replace `BrainOff/BrainOn` and `opposite()` with the ordered C0–C4 enum and explicit contrast definitions.
- [ ] Replace pair-only records with a repeated-measures block containing all five conditions while retaining backward deserialization for schema v1 artifacts.
- [ ] Generalize execution templates from `brain_off/brain_on` to a condition map validated to contain exactly C0–C4 for both harnesses.
- [ ] Test deterministic five-condition randomization, balance of every order position, unique sample IDs, stable seed behavior, and complete blocks.
- [ ] Bump benchmark artifact schemas to v2 without changing the dashboard meaning of old v1 reports.
- [ ] Run the model and matrix focused tests.
...
- [ ] Add `elapsed_ms` to `ProcessOutput`, measured with `Instant` around spawn through final output collection, including timeout cleanup.
- [ ] Parse turns, total tool calls, tool calls by name, Brain MCP calls, CodeGraph calls, file-read calls, unique files, edit calls, and test commands from preserved native output.
- [ ] Record timestamps for first correct file, first edit, and first passing test when the task’s automated trace markers exist; otherwise store `not_observable`, never zero.
- [ ] Test that Claude/Codex cumulative usage is not double counted and timing is not reconstructed from timestamps with different clocks.
- [ ] Preserve raw stdout/stderr content-addressed before normalization.
- [ ] Run `cargo test -p brain-cli --test token_benchmark_trace --test token_benchmark_e2e`.
...
- [ ] Add an append-only `lifecycle_events` table keyed by project plus immutable event UUID; never update a prior stage.
- [ ] Add an append-only `retrieval_decisions` table carrying channel, outcome, reason code, candidate/selected/dropped counts, token count, latency, query hash, and selected evidence IDs.
- [ ] Add project/session/time-bounded reads and fold functions; reject another project’s writes and reads.
- [ ] Define stable reason codes for short prompt, missing session ID, session memory cap, no relevant candidate, budget drop, retrieval error, timeout, and not requested.
- [ ] Prove duplicate event idempotency, conflicting duplicate refusal, source isolation, and old-ledger migration.
- [ ] Run `cargo test -p brain-store --test lifecycle`.
...
- [ ] Record `hook_received` before routing, as the current log does, and prove telemetry failure cannot block the hook.
- [ ] Record retrieval decisions for every SessionStart/UserPromptSubmit, including healthy silence.
- [ ] Record `reply_flushed` only after the pipe write and flush succeed, alongside the existing context-delivery receipt when text exists.
- [ ] Instrument every Brain MCP tool request/success/failure independently from prompt hooks.
- [ ] Require an exact native-session/correlation identifier where the harness can supply one; retain an explicit `unattributed` state where it cannot.
- [ ] Record SessionEnd boundary persistence and capture-caught-up state without claiming EOF when the source is still growing.
- [ ] Rename the 20-memory session cap in constants, telemetry, and tests; do not change its value in the current-state build.
- [ ] Remove stale source comments/tests that still claim Codex Desktop does not fire SessionStart; keep the evidence-backed hook contract in `AGENTS.md` authoritative.
- [ ] Inject write, pipe, retrieval, and capture failures and assert exact lifecycle stages and fail-open behavior.
- [ ] Run the focused service/MCP tests.
...
- [ ] Implement deterministic SessionStart compilation, prompt-push decision, and on-demand search evaluation at an `as_of` cutoff.
- [ ] Score precision/recall/F1, MRR, nDCG@5, fact accuracy, citation precision/coverage, faithfulness, freshness, abstention, harmful push, and healthy silence.
- [ ] Resolve every accepted citation to the actual event/memory and transcript source offset before crediting it.
- [ ] Refuse gold records whose expected source is unreadable, superseded incorrectly, outside the project, or newer than the cutoff.
- [ ] Emit per-case JSONL and aggregate percentages; keep calibration and locked-test commands separate.
- [ ] Add a `brain benchmark retrieval` CLI path that is dry/local and records the executable/config hashes.
- [ ] Run `cargo test -p brain-cli --test retrieval_benchmark`.
...
- [ ] Implement task-clustered estimates for token ratio-of-sums, wall-time ratio-of-sums, pass-rate differences, and retrieval percentages for each contrast.
- [ ] Add Holm-adjusted intervals and deterministic boundary tests at 0%, 10%, −2 percentage points, and +5 percentage points.
- [ ] Emit separate token, speed, and quality statuses plus an overall statement that never hides a failed goal.
- [ ] Keep failed-quality samples in token/time denominators; keep timed-out samples in failure-rate and capped-time sensitivity analyses.
- [ ] Report intention-to-treat and MCP adoption diagnostics separately.
- [ ] Test conflicting per-harness directions, missing condition blocks, invalid native usage, critical regressions, and `not_run` rendering.
- [ ] Run the focused statistics/report tests.
...
- [ ] Validate the exact cumulative difference table C0–C4; reject any extra model, effort, permission, native-memory, skill, provider, or non-Brain hook difference.
- [ ] Strip project Brain/CodeGraph wiring identically, then add only the assigned benchmark capability outside live settings.
- [ ] Clone a fresh native harness home, checkout, Brain home, pipe, and writable CodeGraph state for each sample.
- [ ] Hash production settings before and after every sample and abort on any mutation.
- [ ] Verify C0 has no reachable Brain binary, MCP, hook, environment endpoint, CodeGraph tool, or project instruction asking for them.
- [ ] Verify C1 has CodeGraph and no Brain path; C2 adds only SessionStart/SessionEnd; C3 adds only UserPromptSubmit; C4 adds only Brain MCP.
- [ ] Replace the old `treatment_delivery_observed` check with condition-specific lifecycle assertions, including healthy silence.
- [ ] Run preflight tests and `brain benchmark preflight` without `--execute`.
...
- [ ] Add `brain sessions status --active --closed --json` with project scoping, bounded pagination, and RFC 3339 timestamps.
- [ ] Fold lifecycle events into active, stale-open, closed, and historical-uninstrumented sessions.
- [ ] Add independent startup, prompt-push, MCP-pull, and SessionEnd/capture channel states.
- [ ] Bump dashboard schema to v3 and include bounded active sessions, recent closed sessions, active alerts, and three-goal benchmark summaries.
- [ ] Preserve old delivery aggregates; label them delivery counts, not hook invocation counts.
- [ ] Test service-down, untrusted/missing hook, healthy silence, full delivery, truncation, MCP not requested, MCP failure, boundary stored, capture caught up, stale-open, and cross-project refusal.
- [ ] Run the focused CLI/dashboard tests.
...
- [ ] Add an idempotent `brain install-mcp --harness claude` command that invokes Claude Code’s supported MCP configuration interface at user scope; never hand-edit or replace `~/.claude.json`.
- [ ] Resolve and pin the installed `BRAIN_HOME/bin/brain-mcp.exe`; reject `target/release` and a missing/stale binary.
- [ ] Add a symmetric uninstall path that removes only the server named `brain` after confirming its command matches Agent Brain.
- [ ] Extend the config panel to inspect Claude’s effective MCP wiring as well as Codex’s and report configured, approved/connected, command path, and binary-present states without reading secrets.
...
- [ ] Prove Claude search results remain project-scoped and that an unknown/foreign project or evidence UUID is refused.
- [ ] Record MCP request/success/failure telemetry with Claude harness and exact session correlation when available; use the explicit `unattributed` state otherwise.
- [ ] Keep installation an explicit operator action. Project registration remains one local-path command because the user-scoped MCP server is global and project selection happens inside every tool request.
- [ ] Run `cargo test -p brain-cli --test claude_mcp_install` and the dashboard/config-panel tests.
...
- [ ] Mirror dashboard schema v3 and make every new field optional for cached v2 snapshots.
- [ ] Add active, stale-open, and closed tabs with per-channel status and last-observed timestamps.
- [ ] Render healthy silence and not requested as neutral/healthy states, never zero/failure.
- [ ] Show exact repair guidance for service stopped, binary drift, Codex trust missing, reply-flush failure, and capture lag.
- [ ] Render three independent benchmark cards with C4−C0 headline and incremental contrast drill-downs; `not_run` must say `Not measured`.
- [ ] Poll/push at no more than a 10-second freshness target and label it near-real-time rather than live streaming.
- [ ] Run `pnpm test`, `pnpm lint`, and `pnpm build` in the dashboard repository.

### Task 12: Freeze current behavior and run the non-claimable component pilot

- [ ] Run all workspace unit/integration tests and the 500-case LongMemEval-S evaluation at the pinned current-behavior commit.
...
- [ ] Show SessionStart recency/BM25 orientation, UserPromptSubmit BM25+vector push, and explicit four-channel Brain pull as three distinct paths.
- [ ] State actual Claude/Codex hook and MCP parity from preflight rather than from historical notes.
- [ ] Add measured C0–C4 percentages only when the run is claimable; otherwise show `Not measured` or `Pilot — non-claimable`.
- [ ] Document the lifecycle truth table, near-real-time limitation, alert meanings, and Codex trust-first diagnosis.
- [ ] Mirror project-registration instructions in `AGENTS.md` and `CLAUDE.md`, with `docs/registering-a-project.md` remaining authoritative.
